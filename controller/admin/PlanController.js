...
const softDeletePlan = async (req,res) => {
  try {
    ...
    let updatedPlan = await dbService.updateOne(Plan, query, updateBody);
    if (!updatedPlan){
      return res.recordNotFound();
    }
    return res.success({ data:updatedPlan });
...
const deletePlan = async (req,res) => {
  try { 
    ...
    const query = { _id:req.params.id };
    const deletedPlan = await dbService.deleteOne(Plan, query);
    if (!deletedPlan){
      return res.recordNotFound();
    }
...
const deleteManyPlan = async (req, res) => {
  try {
    ...
    return res.success({ data :{ count :deletedPlan } });
  } catch (error){
    return res.internalServerError({ message:error.message }); 
  }
};
...
const softDeleteManyPlan = async (req,res) => {
  try {
    ...
    let updatedPlan = await dbService.updateMany(Plan,query, updateBody);
    if (!updatedPlan) {
      return res.recordNotFound();
    }
    return res.success({ data:{ count :updatedPlan } });
